...
public:
    ...
    void sync_mpi() {
        // La taille totale du buffer (m_stride * m_stride cellules * 2 phéromones par cellule)
        int total_elements = m_stride * m_stride * 2;
        
        // MPI_Allreduce combine les buffers de tous les processus. 
        // L'opération MPI_MAX garde le taux de phéromone le plus fort parmi tous les processus.
        MPI_Allreduce(MPI_IN_PLACE, &m_buffer_pheronome[0][0], total_elements, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    }
    ...
    void mark_pheronome( const position_t& pos ) {
        ...
        double new_v1 = m_alpha * std::max( {v1_left, v1_right, v1_upper, v1_bottom} ) +
                        ( 1 - m_alpha ) * 0.25 * ( v1_left + v1_right + v1_upper + v1_bottom );
                        
        double new_v2 = m_alpha * std::max( {v2_left, v2_right, v2_upper, v2_bottom} ) +
                        ( 1 - m_alpha ) * 0.25 * ( v2_left + v2_right + v2_upper + v2_bottom );

        std::size_t idx = ( i + 1 ) * m_stride + ( j + 1 );

        // Mise à jour atomique pour éviter les conditions de course
        #pragma omp atomic write
        m_buffer_pheronome[idx][0] = new_v1;

        #pragma omp atomic write
        m_buffer_pheronome[idx][1] = new_v2;
    }
