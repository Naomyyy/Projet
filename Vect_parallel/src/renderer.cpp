...
// O construtor agora recebe a referência para a classe 'ants' (vectorizada)
Renderer::Renderer( const fractal_land& land, const pheronome& phen, 
    ...
    :   m_ref_land( land ),
        ...
        m_ref_ants( ants_ref )
{
    // A textura continua sendo criada no primeiro display() para ter acesso ao SDL_Renderer
}
...
void Renderer::display( Window& win, std::size_t const& compteur )
{
    ...
    // 1. Criação da textura do terreno (Landscape)
    if ( m_land == nullptr ) {
    ...
    // 3. Desenho do terreno nos dois quadrantes superiores
    ...
    // 4. DESENHO DAS FORMIGAS (Versão Vectorizada)
    // Acessamos os dados contíguos na memória (SoA)
    win.set_pen( 0, 255, 255 ); // Cor Ciano para as formigas
    for ( size_t i = 0; i < m_ref_ants.m_data.pos_x.size(); ++i ) {
        // Renderizamos cada formiga no primeiro quadrante
        win.pset( m_ref_ants.m_data.pos_x[i], m_ref_ants.m_data.pos_y[i] );
    }
    
    // 5. Desenho dos Phéromones (Quadrante superior direito)
    for ( fractal_land::dim_t i = 0; i < m_ref_land.dimensions( ); ++i ) {
        for ( fractal_land::dim_t j = 0; j < m_ref_land.dimensions( ); ++j ) {
    ...
    // 6. Desenho da curva de performance (Enfouragement)
