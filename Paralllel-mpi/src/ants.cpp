...
ants::ants(std::size_t nb_ants, const fractal_land& land, unsigned int base_seed) {
    ...
    for (std::size_t i = 0; i < nb_ants; ++i) {
        // Garantimos que cada formiga em cada processo tenha uma semente única
...
void ants::advance_all(pheronome& phen, const fractal_land& land,
    ...
    // Cada processo MPI paraleliza suas próprias formigas com OpenMP
    #pragma omp parallel for reduction(+:local_food_counter)
    for (std::size_t i = 0; i < m_data.pos_x.size(); ++i) {
        ...
        while (consumed_time < 1.0) {
            ...
            if ((choix > eps) || (max_phen <= 0.)) {
                ...
                if (phen(cur_x - 1, cur_y)[ind_pher] == max_phen) next_x -= 1;
                else if (phen(cur_x + 1, cur_y)[ind_pher] == max_phen) next_x += 1;
                else if (phen(cur_x, cur_y - 1)[ind_pher] == max_phen) next_y -= 1;
                else next_y += 1;
            }

            consumed_time += land(next_x, next_y);
            
            // Mark pheronome localmente no buffer do processo
            ...
            if (m_data.pos_x[i] == pos_nest.x && m_data.pos_y[i] == pos_nest.y) {
                if (m_data.states[i] == 1) local_food_counter += 1;
                m_data.states[i] = 0;
            }
            if (m_data.pos_x[i] == pos_food.x && m_data.pos_y[i] == pos_food.y) m_data.states[i] = 1;
        }
    }
}